...
const [sidebarOpen, setSidebarOpen] = useState(false);
  ...
  return (
    ...
    className='flex justify-between items-center px-4 sm:px-12 lg:px-24 xl:px-40 py-4 sticky top-0 z-20 backdrop-blur-xl font-medium bg-white/50 dark:bg-gray-900/70' >
      ...
      <div  className={`text-gray-700 dark:text-white sm:text-sm  max-sm:fixed top-0 bottom-0 right-0 ${!sidebarOpen ? 'max-sm:w-0 overflow-hidden' :   'max-sm:w-[60%] max-sm:pl-20' }max-sm:fixed top-0 bottom-0 right-0 max-sm:min-h-screen max-sm:h-full max-sm:flex-col max-sm:bg-primary max-sm:text-white max-sm:pt-20 flex sm:items-center gap-5 transition-all`} >
        ...
        <a  onClick={()=>setSidebarOpen(false)} href="#contact-us" className='sm:hover:border-b'  >Contact Us</a>
          ...
          <div className='flex items-center gap-2 sm:gap-4' >
           ...
           <img src={theme === 'dark' ? assets.menu_icon_dark : assets.menu_icon} alt=""  onClick={()=>setSidebarOpen(true)}   className='w-8 sm:hidden' />
            ...
            <a href="#contact-us"  className='text-sm max-sm:hidden flex items-center gap-2 bg-primary text-white px-6 py-2 rounded-full cursor-pointer hover:scale-106 transition-all' >Connect  <img src={assets.arrow_icon}  width={14} alt="" /> 
            </a>
